...
def fit_ellipse(x, y):
    ...
    # 3 possible ak's, but only one corresponds to a positive lambda 
    # SOMEBODY PROVE THIS WTF
    ak = eigvec[:, np.nonzero(cond > 0)[0]]
...
def get_ellipse_pts(params, npts=100, tmin=0, tmax=2*np.pi):
    ...
    t = np.linspace(tmin, tmax, npts) # gives a list of ts
    x = x0 + ap * np.cos(t) * np.cos(phi) - bp * np.sin(t) * np.sin(phi)
    y = y0 + ap * np.cos(t) * np.sin(phi) + bp * np.sin(t) * np.cos(phi)

    # x y are lists of values
    return x, y
...
if __name__ == '__main__':
    ...
    # This part find points on an already existing ellipse and adds noise to them
